...
if (isset($_POST['destination_category_id']) || (!isset($_POST['destination_category_id']) && !Category::has_contacts($id))) {
   if (isset($_POST['destination_category_id'])) {
      Contact::change_category($id, intval($_POST['destination_category_id']));
   }
   $category->delete();
   $_SESSION["flash"] = ["message" => "Category removed successfully."];
   header("Location: categories.php");
   return;
} else {
   $has_contacst = true;
